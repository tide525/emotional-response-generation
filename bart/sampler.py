...
class TaskCompetenceBatchSampler(Sampler):
    def __init__(
        self,
        data_source,
        batch_size,
        ...
        self.difficulties = torch.as_tensor(difficulties, dtype=torch.double)
        assert len(self.difficulties) == self.num_tasks
        self.init_competence = init_competence
        self.form = form
        assert self.form in ['linear', 'sqrt']

    def __iter__(self):
        num_tensor = torch.zeros(self.num_tasks, dtype=torch.int64)
        for data in self.data_source:
            task_idx = self.tasks.index(data['task'])
            num_tensor[task_idx] += 1
        ...
        for batch_idx in range(self.num_batches):
            ...
            task_idx = torch.randint(i_max, (1,)).item()
            yield torch.randint(
                low=cum_tensor[task_idx],
                high=cum_tensor[task_idx + 1],
                size=(
                    min(
                        self.batch_size,
                        self.num_samples - batch_idx * self.batch_size
                    ),
                ),
                dtype=torch.int64
            ).tolist()
